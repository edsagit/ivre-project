...
AFRAME.registerComponent('membrane', {
    ...
    init: function () {
      ...
      // marker membrane name
      markerMembraneName = document.querySelector('#membraneName');
      // marker membrane text
      markerMembraneValue = document.querySelector('#membraneValue');
      
      // marker Bpm 
      markerMembraneOctaves = document.querySelector('#membraneOctaves');
      // marker Bpm text
      markerMembraneOctavesText = document.querySelector('#membraneOctavesText');
      // marker Bpm ring
      markerMembraneOctavesRing = document.querySelector('#membraneOctavesRing');
    ...
    tick: function (time, timeDelta) {
      ...
      bpm = Math.abs(map(markerMembrane.object3D.rotation.y, -1.2, 1.2, 40, 300)); // Map marker Y axis rotation to bpm
      octaves = Math.abs(map(markerMembraneOctaves.object3D.rotation.y, -1.2, 1.2, 0.5, 8)); // Map marker Y axis rotation to bpm
      ...
      oscillatorFrequency = Math.abs(map(markerOscillatorFreq.object3D.rotation.y, -1.2, 1.2, 0, 200)); // Map marker Y axis rotation to freq
      oscillatorCount = Math.abs(map(markerOscillatorCount.object3D.rotation.y, -1.2, 1.2, 0, 3)); // Map marker Y axis rotation to freq
      ...
      // set marker text
      markerMembraneName.setAttribute('value', 'Membrane'); 
      // set marker value
      markerMembraneValue.setAttribute('value', 'BPM: ' + Math.round(bpm)); 

      // marker bpm
      // set marker text
      markerMembraneOctavesText.setAttribute('value', 'OCTAVES: ' + Math.round(octaves)); 
      // set a-ring bpm theta value
      markerMembraneOctavesRing.setAttribute('theta-length', map(Math.round(octaves), 0.5, 8, 0, 360)); 

      // marker frequency
      // set marker text
      markerMembraneFreqValue.setAttribute('value', 'FREQ: ' + Math.round(membraneFreq)); 
      ...
      // marker count
      // set marker text
      markerMetalCountValue.setAttribute('value', 'COUNT: ' + Math.round(metalCount));
      // set marker ring
      markerMetalCountRing.setAttribute('theta-length', map(Math.round(metalCount), 0, 16, 0, 360)); 

      // marker decay
      // set marker text
      markerMetalDecayValue.setAttribute('value', 'DECAY: ' + Math.round(metalDecay));
      // set marker ring
      ...
      markerOscillatorFreqValue.setAttribute('value', 'FREQ: ' + Math.round(oscillatorFrequency));
      markerOscillatorFreqRing.setAttribute('theta-length', map(Math.round(oscillatorFrequency), 0, 1000, 0, 360)); 

      markerOscillatorCountValue.setAttribute('value', 'WAVE: ' + Math.round(oscillatorCount));
      markerOscillatorCountRing.setAttribute('theta-length', map(Math.round(oscillatorCount), 0, 3, 0, 360)); 
...
function song(time) {
  ...
  if (Math.round(oscillatorCount) == 0) {
    toneOscillator.type = "sine";
  }
  if (Math.round(oscillatorCount) == 1) {
    toneOscillator.type = "triangle";
  }
  if (Math.round(oscillatorCount) == 2) {
    toneOscillator.type = "square";
  }
  if (Math.round(oscillatorCount) == 3) {
  ...
  membraneSynth.envelope.decay = octaves;
